...
class ModelOptNvFp4FusedMoEMethod(FusedMoEMethodBase):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        already_trtllm_shuffled = (
            self.enable_flashinfer_trtllm_moe
            and hasattr(layer, "gemm1_weights_fp4_shuffled")
            and not hasattr(layer, "w13_weight")
        )
        missing_raw_weights = not hasattr(layer, "w13_weight") or not hasattr(
            layer, "w2_weight"
        )
        if missing_raw_weights and not already_trtllm_shuffled:
            logger.warning_once(
                "Skipping NVFP4 MoE post-processing: missing w13_weight/w2_weight"
            )
            return
        ...
        block_size = 16
        # Validate block weight scales (FP8), not per-tensor *_scale_2.
        assert_dim = 2 if layer.moe_runner_config.is_gated else 1

        def _get_block_weight_scale(name: str):
            scale = getattr(layer, f"{name}_weight_scale", None)
            if scale is not None and scale.dtype == torch.float8_e4m3fn:
                return scale
            return None

        for name in ("w13", "w2"):
            weight_scale = _get_block_weight_scale(name)
        ...
        if already_trtllm_shuffled:
            ...
            if NVFP4_PERTOKEN_SCALE:
                _copy_or_rebind_param(
                    layer,
                    "pertoken_g1_scale_c",
                    (layer.w2_input_scale_quant * w13_weight_scale_2).to(torch.float32),
                )
                _copy_or_rebind_param(
                    layer,
                    "pertoken_g1_alphas",
        ...
        if (
            ...
            if NVFP4_PERTOKEN_SCALE:
                # g1_scale_c keeps the 1/w2_input_scale fold: it is cubin's
                # contract that BF16 GEMM1 output is pre-normalized by a2.
                # g1_alphas applies to the gate half (no GEMM2 re-quant).
                # g2_alphas must include a2 to cancel the a2 normalization
                # carried by BF16_swiglu (otherwise final output is off by
                # 1/a2 per expert -> ~0.3 train-infer logprob diff).
                _copy_or_rebind_param(
                    layer,
                    "pertoken_g1_scale_c",
                    (layer.w2_input_scale_quant * w13_weight_scale_2).to(torch.float32),
            ...
            # Process w13 weights
            w13_blockscale = _get_block_weight_scale("w13")
            if w13_blockscale is not None:
                w13_blockscale_swizzled = swizzle_blockscale(w13_blockscale)
                _copy_or_rebind_param(
            ...
            if intermediate_size_pad:
                ...
                w2_blockscale = _get_block_weight_scale("w2")
                if w2_blockscale is not None:
                    _copy_or_rebind_param(
                        layer,
                        "w2_weight_scale",
                        torch.nn.functional.pad(
                            w2_blockscale, (0, intermediate_size_pad // 16)
                        ),
                    )
    ...
    def apply(
        ...
        if NVFP4_PERTOKEN_SCALE and self.enable_flashinfer_trtllm_moe:
            ...
            quant_info = FlashInferTrtllmFp4MoeQuantInfo(
                ...
                g1_scale_c=layer.pertoken_g1_scale_c.data,
                g1_alphas=layer.pertoken_g1_alphas.data,
                g2_alphas=layer.pertoken_g2_alphas.data,
